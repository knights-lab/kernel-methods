doOuterCV = function(n){
	...
	pearsons = cor(as.numeric(as.logical(knn.prediction)),as.numeric(uni.validation.outcomes))
	res = confusionMatrix(knn.prediction,uni.validation.outcomes)
	index = index+1
	results[index,] = c('KNN.unifrac',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons, paste('k:',uni.knn.model$k))
	#Unifrac RF
	...
	index = index+1
	results[index,] = c('RF.unifrac',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons, '')
	#BRAY_CURTIS
	print('BRAY_CURTIS')
	#----------
	#grab the holdout
	...
	pearsons = cor(as.numeric(svm.prediction),as.numeric(bc.validation.outcomes))
	index = index+1
	results[index,] = c('SVM.bc',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons, paste('kernel:',bc.svm.model$kernel,'C:',bc.svm.model$svm@param$C))
	#KNN
	print('--knn')
	...
	pearsons = cor(as.numeric(as.logical(knn.prediction)),as.numeric(bc.validation.outcomes))
	res = confusionMatrix(knn.prediction,bc.validation.outcomes)
	index = index+1
	results[index,] = c('KNN.bc',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons, paste('k:',bc.knn.model$k))
	#Bray curtis RF
	...
	index = index+1
	results[index,] = c('RF.bc',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons, '')
	#L2 OTU
	print('L2')
	#-------
	#grab the holdout
	...
	pearsons = cor(as.numeric(svm.prediction),as.numeric(l2.validation.outcomes))
	...
	index = index+1
	results[index,] = c('SVM.l2',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons,paste('kernel:',l2.svm.model$kernel,'C:',l2.svm.model$svm@param$C))
	#KNN
	print('--knn')
	...
	pearsons = cor(as.numeric(as.logical(knn.prediction)),as.numeric(l2.validation.outcomes))
	res = confusionMatrix(knn.prediction,l2.validation.outcomes)
	index = index+1
	results[index,] = c('KNN.l2',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons,paste('k:',l2.knn.model$k))
	#L2 RF
	print('--rf')
	...
	index = index+1
	results[index,] = c('RF.l2',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons, '')
	results
	#RAW OTU 
	print('RAW OTU')
	#-------
	...
	index = index+1
	results[index,] = c('RF.otu',res$table[1,1],res$table[2,2],res$table[1,2],res$table[2,1], pearsons, '')
	results
}